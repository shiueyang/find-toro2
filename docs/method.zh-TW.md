...
### 9.5 CJ 的失追不是合併；長視窗 A/m 顯示 DD 與 CJ 的姿態都在變（2026-09-17）

取得全部物件 2025-12-16 ~ 2026-04-08 的根數（44,230 組）後：

- **CJ 失追前沒有鄰居可併**：把所有物件推到 CJ 最後一組根數的歷元（2026-04-07 04:45 UTC），最近的是 IRIDE FM6，相距 119 km、
  沿軌 1.0°，且平均運動差 0.019 rev/day（不同高度）；DD 不在前 15 名內。4 月 1~20 日也沒有任何未識別物件的平均運動跳變。
  結論：CJ 是被 18 SDS 單純失追的獨立物件，不是 DD 的重複編號。
- **長視窗（2026-01-10 ~ 04-05）A/m 與短視窗（2025-12-19 ~ 01-08）比較**：

| 物件 | 12-19 ~ 01-08 | 01-10 ~ 04-05 | 變化 |
|--|--|--|--|
| PHASMA-LAMARR (H) / DIRAC (R) | 0.0210 / 0.0242 | 0.0205 / 0.0229 | −2% / −5%（穩定） |
| AB / CZ（TRYAD 對） | 0.0144 / 0.0147 | 0.0133 / 0.0135 | −8% / −8% |
| CX、L、AA、CY、DM、CN、WISDOM A、BRO-17、T.MicroSat-1 | — | — | 全部在 ±7% 內 |
| **DD** | 0.0185 | **0.0153** | **−17%** |
| **CJ** | 0.0136 | **0.0193** | **+42%** |

被動物件的 A/m 在兩個視窗間都很穩定（連 PHASMA 雙星都在 5% 內），只有 DD 與 CJ 明顯改變，方向相反：
DD 從接近「大面迎風」(0.0182) 掉向「隨機翻滾」(0.0128)，CJ 從「隨機翻滾」升到「大面迎風」以上（0.0193 ≈ 面迎風值 × 1.06，
Cd 略高於 2.2 即可解釋）。這正是**沒有姿態控制、轉動狀態仍在演化**的衛星才會有的特徵，兩顆都符合 TORO-2 的處境，
而兩者的值也都落在 TORO-2 CAD 的 [0.0128, 0.0182] 區間附近。A/m 仍無法在 DD 與 CJ 之間裁決；DD 有 TLE，先追 DD。
